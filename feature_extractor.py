...
def parse_args():
    ...
    parser.add_argument('-j', '--workers', default=8, type=int) # 修改：debug时调整为1
    parser.add_argument('--cos', action='store_true', help='use cosine lr schedule')

    parser.add_argument('--resume', default='', type=str, help='path of model to resume')
    parser.add_argument('--pretrain', default='', type=str, help='path of pretrained model')
    parser.add_argument('--test',
                        default='/home/chenqingzhong/data/chenqingzhong/code/CoCLR/log-pretrain/infonce_k65536_endo240-128_s3d_bs16_lr0.001_seq2_len32_ds1_e700_schedule450_randomframe/model/model_best_epoch176.pth.tar',
                        type=str, help='path of model to load and pause')
    # parser.add_argument('--test',
    #                     default='/home/chenqingzhong/data/chenqingzhong/code/CoCLR/log-pretrain/shared/InfoNCE-ucf101-rgb-128-s3d-ep399.pth.tar',
    #                     type=str, help='path of model to load and pause')

    parser.add_argument('--retrieval', default=True, action='store_true', help='path of model to ucf retrieval')
...
def main(args):
    ...
    ### classifier model ###
    num_class_dict = {'endo240': 59, 'ucf101': 101, 'hmdb51': 51, 'k400': 400,
...
def test_retrieval(model, criterion, transforms_cuda, device, epoch, args):
    ...
    def tr(x):
        ...
        # 修改代码，也是特征提取阶段所用代码：取所有帧
        test_sample = 1
        return transforms_cuda(x) \
            .view(3, test_sample, args.num_seq, x.size(2), args.img_dim, args.img_dim).permute(1, 2, 0, 3, 4, 5)
    ...
    with torch.no_grad():
        ...
        if args.dirname is None:
            dirname = 'feature_epoch%s' % args.test.split('epoch')[1].split('.')[0]
        else:
            dirname = args.dirname
        feature_folder = os.path.join(os.path.dirname(args.test), dirname)
        ...
        #         os.path.join(os.path.dirname(args.test), dirname, '%s_test_feature.pth.tar' % args.dataset)).to(device)
        #     test_label = torch.load(
        #         os.path.join(os.path.dirname(args.test), dirname, '%s_test_label.pth.tar' % args.dataset)).to(device)
        # else:
        #     try:
        ...
        if os.path.exists(os.path.join(os.path.dirname(args.test), dirname, '%s_train_feature.pth.tar' % args.dataset)):
            train_feature = torch.load(
                os.path.join(os.path.dirname(args.test), dirname, '%s_train_feature.pth.tar' % args.dataset)).to(device)
            train_label = torch.load(
                os.path.join(os.path.dirname(args.test), dirname, '%s_train_label.pth.tar' % args.dataset)).to(device)
        else:
            ...
            for idx, (input_seq, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
                ...
                # 修改：保存feature为npy文件
                feature_name = vname[0].split('/')[-2]
                np.save(os.path.join(os.path.dirname(args.test), dirname, '%s.npy' % feature_name), feature.cpu())
...
def get_data(transform, mode, args):
    ...
    if args.dataset == 'endo240':
        dataset = ENDO240LMDB(mode=mode, transform=transform,
