...
class STConv3d(nn.Module):
    ...
    def forward(self,x): #(1,3,frame,128,128)
        x=self.conv1(x) #(1,64,frame,64,64)
        x=self.bn1(x)
        x=self.relu(x)
        x=self.conv2(x)
        x=self.bn2(x)
...
class S3D(nn.Module):
    ...
    def forward(self, x): #(1,3,frame,128,128)
        x = self.block1(x) #(1,64,frame/2,64,64)
        x = self.block2(x) #(1,192,frame/2,32,32)
        x = self.block3(x) #(1,480,frame/2,16,16)
        x = self.block4(x) #(1,832,frame/4,8,8)
        x = self.block5(x) #(1,1024,frame/8,4,4)
        return x 
