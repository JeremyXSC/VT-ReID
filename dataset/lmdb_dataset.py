# 训练时使用endo240p_frames_train(line49)、ClassInd.txt(line77)、train_split01.csv(line88)
# 提取特征时，除了对训练帧之外，还需要额外对duplicate帧进行，修改为endo240p_duplicate_frames_train（line49）、ClassInd_duplicate.txt(line78)、train_duplicate_split01.csv(line89)
import os
import sys
import glob
import msgpack
...
__all__ = [
    'ENDO240LMDB_2CLIP', 'UCF101LMDB_2CLIP', 'UCF101Flow_LMDB_2CLIP', 'UCF101_2STREAM_LMDB_2CLIP',
...
lmdb_root = 'D:\Colonpolyps\code\SCRL\data\Colo-SCRL/'
...
class ENDO240LMDB_2CLIP(object):
    def __init__(self, root='%s/../process_data/data/endo240' % os.path.dirname(os.path.abspath(__file__)),
                 db_path=os.path.join(lmdb_root, 'dataset/endo240p_frames_train'),
        ...
        with self.env.begin(write=False) as txn:
            self.db_length = msgpack.loads(txn.get(b'__len__'), raw=True)
            self.db_keys = msgpack.loads(txn.get(b'__keys__'), raw=True)
            self.db_order = msgpack.loads(txn.get(b'__order__'), raw=True)

        classes = read_file(os.path.join(root, 'ClassInd.txt'))
        # classes = read_file(os.path.join(root, 'ClassInd_duplicate.txt'))
        ...
        # video_info = pd.read_csv(os.path.join(root, '%s_duplicate_split%02d.csv' % (split_mode, which_split)), header=None)
    ...
    def __getitem__(self, index):
        ...
        with env.begin(write=False) as txn:
            ...
            # raw = msgpack.loads(txn.get(self.get_video_id[vname].encode('utf-16'))) #修改
        ...
        if self.return_label:
            ...
            vid = 0 # 修改：args.model == 'infonce'时不需要使用到label因此设置为固定值
            if self.return_source:
...
class ENDO240LMDB(ENDO240LMDB_2CLIP):
    def __init__(self, **kwargs):
        super(ENDO240LMDB, self).__init__(**kwargs)
...
class UCF101LMDB(ENDO240LMDB_2CLIP): #修改
    def __init__(self, **kwargs):
    ...
    def __getitem__(self, index):
        ...
        # 原始代码，也是预训练阶段所用代码：时间维度上视频扩充（随机采样连续的32帧）
        # frame_index = self.frame_sampler(vlen)
        # seq = [pil_from_raw_rgb(raw[i]) for i in frame_index]
        # 修改代码1，也是特征提取阶段所用代码：取所有帧
        seq = [pil_from_raw_rgb(raw[i]) for i in range(vlen)]
        # 修改代码2，也是特征提取阶段所用代码：每秒取5帧（还未修改）
        # seq = [pil_from_raw_rgb(raw[i]) for i in range(vlen)]
        ...
        if self.return_label:
            # vid = self.encode_action(vlabel) #修改
            vid = 0
            if self.return_source:
                source_id = self.video_source[vname.split('/')[-1][0:-4]]
                return seq, source_id, vid
...
class UCF101Flow_LMDB_2CLIP(object):
    def __init__(self, root='%s/../process_data/data/ucf101' % os.path.dirname(os.path.abspath(__file__)), 
        ...
        with self.env.begin(write=False) as txn:
            ...
            self.db_keys = msgpack.loads(txn.get(b'__keys__'), raw=True)
            self.db_order = msgpack.loads(txn.get(b'__order__'), raw=True)
            self.vlen_list = msgpack.loads(txn.get(b'__vlen__'), raw=True)
...
class UCF101_2STREAM_LMDB_2CLIP(object):
    def __init__(self, root='%s/../process_data/data/ucf101' % os.path.dirname(os.path.abspath(__file__)), 
        ...
        with self.env_flow.begin(write=False) as txn:
            self.db_length_flow = msgpack.loads(txn.get(b'__len__'), raw=True)
            self.db_keys_flow = msgpack.loads(txn.get(b'__keys__'), raw=True)
            self.db_order_flow = msgpack.loads(txn.get(b'__order__'), raw=True)
            self.vlen_list_flow = msgpack.loads(txn.get(b'__vlen__'), raw=True)
        ...
        with self.env_rgb.begin(write=False) as txn:
            self.db_length_rgb = msgpack.loads(txn.get(b'__len__'), raw=True)
            self.db_keys_rgb = msgpack.loads(txn.get(b'__keys__'), raw=True)
            self.db_order_rgb = msgpack.loads(txn.get(b'__order__'), raw=True)
...
class KineticsLMDB_2CLIP(object):
    def __init__(self, root, db_path, filename,
        ...
        with self.env.begin(write=False) as txn:
            ...
            self.db_keys = msgpack.loads(txn.get(b'__keys__'), raw=True)
            self.db_order = msgpack.loads(txn.get(b'__order__'), raw=True)
            if self.is_flow: self.vlen_list = msgpack.loads(txn.get(b'__vlen__'), raw=True)
