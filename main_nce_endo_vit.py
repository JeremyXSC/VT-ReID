...
def parse_args():
    ...
    parser.add_argument('--videomae_checkpoint',
                        default='/home/chenqingzhong/data/chenqingzhong/code/VideoMAE/endo_pretrain_base_patch16_112_frame_16x4_tube_mask_ratio_0.9_e800/checkpoint-800.pth',
                        help='finetune from checkpoint')

    parser.add_argument('--net', default='vit', type=str)
    ...
    parser.add_argument('--schedule', default=[450, 480], nargs='*', type=int, help='learning rate schedule (when to drop lr by 10x)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.net == 'vit':
        checkpoint = torch.load(args.videomae_checkpoint, map_location='cpu')
        print("Load ckpt from %s" % args.videomae_checkpoint)

        checkpoint_model = checkpoint['model']
        print("Load state_dict by model_key = %s" % 'model')
...
def train_one_epoch(data_loader, model, criterion, optimizer, transforms_cuda, epoch, args):
    ...
    for idx, (input_seq, label) in tqdm(enumerate(data_loader), total=len(data_loader), disable=True):
        ...
        # 修改：用于测试哪些参数没有接收到回传梯度
        # for name, param in model.named_parameters():
        #     if param.grad is None:
        #         print('param.grad is None:', name)
